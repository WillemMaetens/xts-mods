...
xtsAttributes(x) <- list(name = "test series")

# NOTE: expected value is (2 + 2) to account for
#       (1) column header: "             [,1]  [,2]"
#       (2) message: "[ reached getOption("max.print") -- omitted 6 rows ]"
print_output <- utils::capture.output(print(x, max = 4, show.attrs = FALSE))
expect_true(length(print_output) == (2 + 2), info = "'max' argument is respected'")

# NOTE: expected value is (4 + 2 + 2) to account for
#       (1) xtsAttributes
#       (2) column header: "             [,1]  [,2]"
#       (3) message: "[ reached getOption("max.print") -- omitted 6 rows ]"
print_output <- utils::capture.output(print(x, max = 4))
expect_true(length(print_output) == (4 + 2 + 2), info = "'max' argument is respected'")

print_output <- utils::capture.output(print(x, max = 4, show.nrows = 10))
expect_true(length(print_output) == (4 + 2 + 2),
            info = "'max' takes precedence over 'show.nrows'")

expect_no_error(p <- print(drop(x[, 1])),
              message = "print.xts() does not error when object doesn't have dims")

print_output <- utils::capture.output(print(drop(x[1:2, 1]), show.attrs = FALSE))
...
print_output <- utils::capture.output(print(x, show.nrows = 10, trunc.rows = 4, show.attrs = FALSE))
...
print_output <- utils::capture.output(print(y, show.nrows = show_nrows, trunc.rows = nrow(y)-2, show.attrs = FALSE))
...
expect_no_error(print(z), message = "print more columns than width doesn't error")
